...
namespace F4SEVR
{
    ...
    {
        UnpackArray(dst, src, GetTypeID<VMArray<VMObject>>(getGameVM()->m_virtualMachine));
    }

    template <>
    void UnpackValue<VMArray<VMRefOrInventoryObj>>(VMArray<VMRefOrInventoryObj>* dst, VMValue* src)
    {
        UnpackArray(dst, src, GetTypeID<VMArray<VMRefOrInventoryObj>>(getGameVM()->m_virtualMachine));
    }
    ...
    void* UnpackHandle(VMValue* src, UInt32 typeID)
    {
        ...
        if (!getObjectHandlePolicy()->IsType(typeID, handle))
            return NULL;
        if (!getObjectHandlePolicy()->Unk_03(handle))
            return NULL;

        return getObjectHandlePolicy()->Resolve(typeID, handle);
    }
