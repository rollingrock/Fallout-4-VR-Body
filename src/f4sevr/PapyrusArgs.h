...
namespace F4SEVR
{
    ...
    public:
        ...
        DEFINE_MEMBER_FN(GetOffset, UInt32, 0x026D26F0, VMState* state);
        // 4C8A9FE3A95B9DED322C9C0A78312E29F0A2CC7A+5B
        DEFINE_MEMBER_FN(Get, VMValue*, 0x026D2750, VMState* state, UInt32 idx, UInt32 offset);
    };
    ...
    public:
        ...
        {
            PackValue(&m_data[idx], src, getGameVM()->m_virtualMachine);
            if (m_arr && bReference)
                PackValue(&m_arr->arr.entries[idx], src, getGameVM()->m_virtualMachine);
        }

        void Push(T* src, bool bReference = true)
        {
            VMValue tmp;
            PackValue(&tmp, src, getGameVM()->m_virtualMachine);
    ...
    public:
        ...
        {
            PackValue(&m_value, src, getGameVM()->m_virtualMachine);
            if (m_var && bReference)
                PackValue(m_var, src, getGameVM()->m_virtualMachine);
        }
        ...
        bool Is()
        {
            return m_value.type.value == GetTypeID<T>(getGameVM()->m_virtualMachine);
        }
    ...
    public:
        ...
        bool Is()
        {
            if (m_value.IsIdentifier() && m_value.data.id) {
                ...
                return getObjectHandlePolicy()->IsType(BaseType::kTypeID, handle);
            }
        ...
        UInt64 GetHandle()
        {
            ...
            return getObjectHandlePolicy()->GetInvalidHandle();
        }
    ...
    {
        UnpackArray(dst, src, GetTypeID<VMArray<T>>(getGameVM()->m_virtualMachine));
    }
