...
namespace common
{
    ...
    RE::NiTransform getDeltaTransform(const RE::NiTransform& from, const RE::NiTransform& to)
    {
        ...
        delta.translate = to.translate - matrixVecMultTempFix(delta.rotate.Transpose(), from.translate * delta.scale);
        return delta;
    }
    ...
    RE::NiTransform getTargetTransform(const RE::NiTransform& baseFrom, const RE::NiTransform& baseTo, const RE::NiTransform& targetFrom)
    {
        ...
        target.translate = matrixVecMultTempFix(delta.rotate.Transpose(), (targetFrom.translate * delta.scale) + delta.translate);
        return target;
    }
    ...
    bool isCameraLookingAtObject(const RE::NiTransform& cameraTrans, const RE::NiTransform& objectTrans, const float detectThresh)
    {
        ...
        const auto cameraForward = vec3Norm(matrixVecMultTempFix(cameraTrans.rotate.Transpose(), RE::NiPoint3(0, 1, 0)));

        // Get the forward vector of the object (assuming it's the y-axis)
        const auto objectForward = vec3Norm(matrixVecMultTempFix(objectTrans.rotate.Transpose(), RE::NiPoint3(0, 1, 0)));
