...
namespace f4vr {
	...
	inline RelocAddr<std::uint64_t> testin(0x5a3b888);

	using _IsInAir = bool(*)(RE::Actor* actor);
	inline RelocAddr<_IsInAir> IsInAir(0x00DC3230);

	typedef int (*_GetSitState)(RE::BSScript::Internal::VirtualMachine* registry, std::uint64_t stackID, RE::Actor* actor);
	inline RelocAddr<_GetSitState> GetSitState(0x40e410);

	typedef bool (*_IsSneaking)(RE::Actor* a_actor);
	...
	using _Actor_GetAmmoClipPercentage = float(*)(RE::Actor* a_actor, f4vr::BGSEquipIndex a_idx);
	inline RelocAddr<_Actor_GetAmmoClipPercentage> Actor_GetAmmoClipPercentage(0xddf6c0);

	using _Actor_GetCurrentAmmoCount = float(*)(RE::Actor* a_actor, f4vr::BGSEquipIndex a_idx);
	...
	using _AIProcess_CreateMuzzleFlash = void* (*)(Actor::MiddleProcess* middleProcess, uint64_t projectile, RE::Actor* actor);
