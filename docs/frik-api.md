...
> **Writing a new integration?** Use the [FRIK API v2](frik-api-v2.md) instead. It covers everything on this page plus external hand placement, primary weapon node ownership, and visual weapon recoil. This page documents the original v1.\* table, which stays exported and fully supported for existing mods — both APIs can be used side by side and arbitrate through the same tags.
...
This page calls that whole family **v1.\***: each revision is v1.N, where N is `FRIK_API_VERSION`. So v1.1 through v1.4 are the four revisions of this table and v1.4 is current. The separate [v2](frik-api-v2.md) API is a different table with its own version counter; the two never mix.

- `getVersion()` returns the API version FRIK was built with — the digit after the dot, so `4` means v1.4. `getModVersion()` returns the FRIK mod version string (e.g. `"0.78.0"`).
...
`bool setHandPose(const char* tag, Hand hand, HandPoseKind handPose)` — *(since v1.2)*
...
`bool setHandPoseCustomFingerPositions(const char* tag, Hand hand, float thumb, float index, float middle, float ring, float pinky)` — *(since v1.2)*

Each value is `0..1` (`0` = fully bent, `1` = fully straight) applied uniformly across that finger's joints.

`bool setHandPoseCustom(const char* tag, Hand hand, const HandPoseData& handPose, bool forceTop)` — *(since v1.4)*
...
| --- | --- | --- |
| `bool clearHandPose(tag, hand)` | v1.2 | Release this tag's override so FRIK regains control. |
| `HandPoseKind getCurrentHandPose(hand)` | v1.2 | The pose currently active on the hand (v1.4+ returns canonical `HandPoseKind`). |
| `HandPoseTagState getHandPoseSetTagState(tag, hand)` | v1.2 | `None` (not set), `Active` (set and winning), or `Overriden` (set but another tag wins). |

Use `getHandPoseSetTagState` to detect when another system has taken over the pose and react accordingly.

> **Deprecated:** `setHandPoseFingerPositions` / `clearHandPoseFingerPositions` (v1.1) are tagless and share a single legacy slot. Use the tagged `setHandPoseCustomFingerPositions` / `clearHandPose` instead.
...
| Function | Since | Description |
| --- | --- | --- |
| `bool isSkeletonReady()` | v1.1 | FRIK is loaded and the skeleton is initialized. |
| `RE::NiPoint3 getIndexFingerTipPosition(hand)` | v1.1 | World position of the index fingertip. |
| `bool isConfigOpen()` | v1.2 | Any FRIK config UI is open (main, Pip-Boy, or weapon adjustment). |
| `bool isSelfieModeOn()` | v1.2 | Selfie mode state. |
| `void setSelfieModeOn(bool)` | v1.2 | Turn selfie mode on/off. |
| `bool isOffHandGrippingWeapon()` | v1.2 | The weapon is currently held two-handed (offhand on the weapon). |
| `bool isWristPipboyOpen()` | v1.2 | The wrist Pip-Boy is currently open. |
...
`bool blockFeature(const char* tag, Feature feature, bool block)` — *(since v1.4)*
`bool isFeatureBlocked(Feature feature)` — *(since v1.4)*
...
`bool blockOffHandWeaponGripping(const char* tag, bool block)` — *(since v1.3)*
...
`int getConfigValue(section, key, char* outBuf, int bufLen, defaultValue)` — *(since v1.4)*
...
`bool registerOpenModSettingButtonToMainConfig(const OpenExternalModConfigData& data)` — *(since v1.2)*
...
- **Republish hand-pose overrides after a skeleton rebuild.** They are dropped when the skeleton is released. FRIK broadcasts F4SE message type `100` (skeleton ready) and `101` (skeleton destroying) under `FRIK_F4SE_MOD_NAME` — see [API v2](frik-api-v2.md#skeleton-lifecycle). Feature blocks and config overrides are unaffected.
